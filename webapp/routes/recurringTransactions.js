...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.query.upcoming_days) {
      ...
      params.push(parseInt(req.query.upcoming_days));
    }
...
router.post('/',
  ...
  async (req, res) => {
    ...
    // next_due_date starts as start_date on creation
    ...
    try {
      const result = await query(
        `INSERT INTO recurring_transactions
         ...
         RETURNING *`,
        [name, description, amount, currency, type, frequency,
         start_date, end_date || null, next_due_date, category || null,
         tags || null]
      );
...
router.put('/:id',
  ...
  validate,
  async (req, res) => {
    const fields = ['name','description','amount','currency','type','frequency',
...
// Mark as processed → advances next_due_date, creates a bill entry
router.post('/:id/process', async (req, res) => {
  try {
    ...
    // Auto-create a bill for this occurrence
